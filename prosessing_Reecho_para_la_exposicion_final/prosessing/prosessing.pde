...
void draw(){ 
  ...
  if(!grafico && !archivo && !usuario){ // este if sirve para entrar al menu principal de la app
    ...
    fill(30);
    textSize(30);
    text("usuaruio:"+usser,width/2-200,height/2-120);
    
    fill(#50B2A9);
    rect(width/3.2,height/28,230,50,28);
    fill(255);
    textSize(30);
    text("ECG-Arduino",width/3,height/8-20);
    
    fill(#5097B2);
    rect(width/2-230,height/2-50,220,50,28);
    fill(255);
    text("Mirar-ECG",width/2-200,height/2-15);
    
    fill(#5097B2);
    rect(width/2-240,height/2+60,220,50,28);
    fill(255);
    text("Grabaciones",width/2-220,height/2+100);
...
void mousePressed(){
  ...
  */
  ...
  if((mouseX>width/2-240) && (mouseX<width/2) && (mouseY>height/2+60) && (mouseY<height/2+110) && !archivo && !usuario){
     archivo = true;
  }
...
void imagen(){ 
  ...
  image(logo,width/2-450,height/2+150);
  textSize(18);
  text("By Valentino_Montivero.",width/2+250,height/2+280);
